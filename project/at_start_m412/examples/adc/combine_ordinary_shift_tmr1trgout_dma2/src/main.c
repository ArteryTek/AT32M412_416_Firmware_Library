...
/** @addtogroup 412_ADC_combine_ordinary_shift_tmr1trgout_dma2 ADC_combine_ordinary_shift_tmr1trgout_dma2
  * @{
  */
...
static void adc_config(void)
{
  ...
  /* config combine mode */
  adc_common_struct.combine_mode = ADC_ORDINARY_SHIFT_ONLY_ONESLAVE_MODE;
...
int main(void)
{
  ...
  dma_config();
  adc_config();
  printf("combine_ordinary_shift_tmr1trgout_dma2 \r\n");
  ...
  else
  {
    ...
    {
      printf("adccom_ordinary_valuetab[%d] = 0x%x\r\n",index1, adccom_ordinary_valuetab[index1]);
    }
    printf("\r\n");
  }
  at32_led_on(LED2);
