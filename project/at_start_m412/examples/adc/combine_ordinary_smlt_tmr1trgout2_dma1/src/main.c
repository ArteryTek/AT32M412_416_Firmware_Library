...
/** @addtogroup 412_ADC_combine_ordinary_smlt_tmr1trgout2_dma1 ADC_combine_ordinary_smlt_tmr1trgout2_dma1
  * @{
  */

#define DMA_BUFFER_SIZE                  18

__IO uint16_t adccom_ordinary_valuetab[3][6];
__IO uint16_t dma1_trans_complete_flag = 0;
...
static void tmr1_config(void)
{
  ...
  tmr_output_channel_config(TMR1, TMR_SELECT_CHANNEL_5, &tmr_oc_init_structure);
  tmr_output_channel_mode_select(TMR1, TMR_SELECT_CHANNEL_5, TMR_OUTPUT_CONTROL_PWM_MODE_A);
  tmr_channel_value_set(TMR1, TMR_SELECT_CHANNEL_5, 500);
  tmr_channel_enable(TMR1, TMR_SELECT_CHANNEL_5, TRUE);
  tmr_primary_mode2_select(TMR1, TMR_PRIMARY_SEL_OVERFLOW);
...
static void adc_config(void)
{
  ...
  adc_ordinary_channel_set(ADC1, ADC_CHANNEL_2, 1, ADC_SAMPLETIME_41_5);
  ...
  adc_ordinary_channel_set(ADC2, ADC_CHANNEL_1, 1, ADC_SAMPLETIME_41_5);
  adc_ordinary_channel_set(ADC2, ADC_CHANNEL_3, 2, ADC_SAMPLETIME_41_5);
  adc_ordinary_channel_set(ADC2, ADC_CHANNEL_5, 3, ADC_SAMPLETIME_41_5);
...
void adc_convert_recovery_process(void)
{
  ...
  recovery_index = ADCCOM->cctrl & (0x1F << 0);
	
  /* clear adc mode configuration */
  ADCCOM->cctrl &= (uint32_t)~(0x1F << 0);
...
int main(void)
{
  ...
  dma_config();
  adc_config();
  printf("combine_ordinary_smlt_tmr1trgout2_dma1 \r\n");
  tmr_counter_enable(TMR1, TRUE);
  while(dma1_trans_complete_flag == 0);
  tmr_counter_enable(TMR1, FALSE);
  if((adc1_overflow_flag != 0) || (adc2_overflow_flag != 0) || (adc1_conversion_fail_flag != 0) || (adc2_conversion_fail_flag != 0))
  {
  ...
  else
  {
    ...
    for(index1 = 0; index1 < 3; index1++)
    {
      ...
      printf("adccom_ordinary_valuetab[%d][2] = 0x%x\r\n",index1, adccom_ordinary_valuetab[index1][2]);
      printf("adccom_ordinary_valuetab[%d][3] = 0x%x\r\n",index1, adccom_ordinary_valuetab[index1][3]);
      printf("adccom_ordinary_valuetab[%d][4] = 0x%x\r\n",index1, adccom_ordinary_valuetab[index1][4]);
      printf("adccom_ordinary_valuetab[%d][5] = 0x%x\r\n",index1, adccom_ordinary_valuetab[index1][5]);
      printf("\r\n");
    }
  }
  at32_led_on(LED2);
