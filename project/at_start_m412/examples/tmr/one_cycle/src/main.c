...
/** @addtogroup 412_TMR_one_cycle TMR_one_cycle
  * @{
  */

//#define RETRIGGER_OCM_MODE
...
int main(void)
{
  ...
  prescalervalue = (uint16_t) ((crm_clocks_freq_struct.sclk_freq) / 1000000) - 1;

  /* tmr base configuration */
  tmr_base_init(TMR1, 5000, prescalervalue);
  tmr_cnt_dir_set(TMR1, TMR_COUNT_UP);
  tmr_clock_source_div_set(TMR1, TMR_CLOCK_DIV1);

  /* output compare toggle mode configuration: channel1 */
  tmr_output_default_para_init(&tmr_oc_init_structure);
  #ifdef RETRIGGER_OCM_MODE
  tmr_oc_init_structure.oc_mode = TMR_OUTPUT_CONTROL_RETRIG_OCM_A;
  #else
  ...
  tmr_channel_value_set(TMR1, TMR_SELECT_CHANNEL_1, 4000);
  ...
  #ifdef RETRIGGER_OCM_MODE
  tmr_sub_mode_select(TMR1, TMR_SUB_RESET_TRIGGER_MODE);
  #else
...
void gpio_configuration(void)
{
  ...
  /* tmr1_ch2 pin (pa.09) configuration */
  ...
  /* tmr1_ch1 pin (pa.08) configuration */
