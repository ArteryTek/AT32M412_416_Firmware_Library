...
/** @addtogroup 412_TMR_external_clock TMR_external_clock
  * @{
  */
...
int main(void)
{
  ...
  /* enable tmr4/tmr3/gpioa/gpiob clock */
  ...
  /* use tmr4 generate clock to drive tmr3--------------------
  tmr4 pwm frequency = (apb1_freq*2)/180(period+1)/1000(div+1) = 1 khz
  tmr3 pwm frequency = 1000(tmr4 pwm frequency)/100(period+1)/1(div+1) = 10 hz
  ------------------------------------------------------------ */
  /* tmr4 and tmr3 counter mode configuration */

  timerperiod = ((crm_clocks_freq_struct.sclk_freq) / 1000000) - 1;
  tmr_base_init(TMR4, timerperiod, 999);
  tmr_cnt_dir_set(TMR4, TMR_COUNT_UP);

  tmr_base_init(TMR3, 99, 0);
  tmr_cnt_dir_set(TMR3, TMR_COUNT_UP);

  /* tmr4 and tmr3 output mode configuration */
  ...
  tmr_channel_value_set(TMR4, TMR_SELECT_CHANNEL_1, timerperiod/2);
  /* tmr3 channel 3 */
