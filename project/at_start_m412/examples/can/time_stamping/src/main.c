...
/** @addtogroup 412_CAN_time_stamping CAN_time_stamping
  * @{
  */
...
error_status can_configuration(void)
{
  ...
  /* time stamping position at sof */
  can_timestamp_position_set(CAN1, CAN_TIMESTAMP_AT_SOF);
  can_timestamp_enable(CAN1, TRUE);
  
  /* can interrupt config */
  nvic_irq_enable(CAN1_RX_IRQn, 0x00, 0x00);
  nvic_irq_enable(CAN1_ERR_IRQn, 0x00, 0x00);
...
static void can_transmit_data(void)
{
  ...
  can_txbuf_struct.tx_timestamp = TRUE;
  ...
  printf("transmit timestamp = %u\r\n", can_transmit_timestamp_get(CAN1));
}

/**
  *  @brief  tmr configiguration.
  *  @param  none
  *  @retval none
  */
static void tmr_configuration(void)
{
  ...
  tmr_reset(TMR4);
  tmr_base_init(TMR4, 0xFFFF, (crm_clocks_freq_struct.apb1_freq / 10000) - 1);
  tmr_cnt_dir_set(TMR4, TMR_COUNT_UP);
  tmr_counter_enable(TMR4, TRUE);
}

/**
  *  @brief  scfg configiguration.
  *  @param  none
  *  @retval none
  */
static void scfg_configuration(void)
{
  crm_periph_clock_enable(CRM_SCFG_PERIPH_CLOCK, TRUE);
  
  scfg_can_timestamp_source_set(SCFG_CAN1, SCFG_CAN_TIMESTAMP_TMR4);
}
