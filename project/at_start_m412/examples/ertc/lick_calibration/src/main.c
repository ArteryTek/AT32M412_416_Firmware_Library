...
/** @addtogroup 412_ERTC_lick_calibration ERTC_lick_calibration
  * @{
  */
...
int main(void)
{
  ...
  /* get the lick frequency: TMR11 is used to measure the lick frequency */
...
/**
  * @brief  configures TMR11 to measure the lick oscillator frequency.
  * @param  none
...
uint32_t lick_frequency_get(void)
{
  ...
  /* enable TMR11 apb1 clocks */
  crm_periph_clock_enable(CRM_TMR11_PERIPH_CLOCK, TRUE);

  /* connect internally the TMR11_ch1 to the lick clock output */
  tmr_input_ch_select(TMR11, TMR_TMR11_CH1_ERTCCLK);

  /* configure TMR11 divider */
  tmr_div_value_set(TMR11, 0);
  tmr_event_sw_trigger(TMR11, TMR_OVERFLOW_SWTRIG);

  /* TMR11 channel4 input capture mode configuration */
  ...
  tmr_input_channel_divider_set(TMR11, TMR_SELECT_CHANNEL_1, TMR_CHANNEL_INPUT_DIV_8);

  /* enable TMR11 interrupt channel */
  nvic_irq_enable(TMR1_TRG_HALL_TMR11_IRQn, 0, 0);

  /* enable TMR11 counter */
  ...
  tmr_interrupt_enable(TMR11, TMR_C1_INT, TRUE);

  /* wait the TMR11 measuring operation to be completed */
  while(capture_number != 2);

  /* deinitialize the TMR11 peripheral */
  tmr_reset(TMR11);
