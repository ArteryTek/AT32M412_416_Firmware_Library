...
/** @addtogroup 416_TMR_pwm_output_asym TMR_pwm_output_asym
  * @{
  */

//#define INDEPENDENT_ASYM
...
int main(void)
{
  ...
  channel2pulse = (uint16_t) (((uint32_t) 70 * (timerperiod + 1)) / 100);
  channel3pulse = channel1pulse;
  channel4pulse = channel2pulse;

  tmr_base_init(TMR1, timerperiod, 0);
  tmr_cnt_dir_set(TMR1, TMR_COUNT_TWO_WAY_1);

  /* channels 1 and 2 are config to asymmetric pwm mode a */
  tmr_output_default_para_init(&tmr_output_struct);
  tmr_output_struct.oc_mode = TMR_OUTPUT_CONTROL_ASYMMETRIC_PWM_B;
  ...
  tmr_channel_value_set(TMR1, TMR_SELECT_CHANNEL_2, channel2pulse);
	/* for comparison channes 3 and 4 are config to pwm mode a */
  /* channel 3 */
  ...
  #ifdef INDEPENDENT_ASYM
  tmr_iasym_mode_enable(TMR1, TRUE);
  tmr_channel_value_set(TMR1, TMR_SELECT_CHANNEL_1A, channel2pulse);
  tmr_channel_value_set(TMR1, TMR_SELECT_CHANNEL_2A, channel1pulse);
  #endif
