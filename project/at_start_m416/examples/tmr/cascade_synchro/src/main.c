...
/** @addtogroup 416_TMR_cascade_synchro TMR_cascade_synchro
  * @{
  */
...
int main(void)
{
  ...
  /* enable tmr1/tmr3/tmr4/gpioa/gpiob clock */
  ...
  /* timers synchronisation in cascade mode ----------------------------
   ...
   2/tmr3 is sub for tmr1 and primary for tmr4,
   - pwm mode is used
   ...
   - gated mode is used, so start and stop of sub counter
    ...
    tim2 frequency = (tmr1 counter clock)/ (tmr1 period + 1) = 1 mhz
    and the duty cycle = tmr1_ccr1/(tmr1_arr + 1) = 50%.

    the tmr3 is running at:
    (tmr1 frequency)/ (tmr3 period + 1) = 100 khz and a duty cycle equal
