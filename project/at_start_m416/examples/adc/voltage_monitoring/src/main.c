...
/** @addtogroup 416_ADC_voltage_monitoring ADC_voltage_monitoring
  * @{
  */
...
int main(void)
{
  ...
  while(1)
  {
    ...
    delay_sec(1);
    if(error_times_index != (adc1_overflow_flag + adc1_conversion_fail_flag + vmor_flag_index))
    {
      /* printf flag when error occur */
      error_times_index = adc1_overflow_flag + adc1_conversion_fail_flag + vmor_flag_index;
      at32_led_on(LED3);
      at32_led_on(LED4);
      printf("error occur\r\n");
      ...
      printf("out of range:adc1_channel_3 value is = 0x%x!\r\n", adc1_ordinary_valuetab[1]);
    }
