...
void CAN1_RX_IRQHandler(void)
{
  ...
  if(can_interrupt_flag_get(CAN1, CAN_RIF_FLAG) != RESET)
  {
    ...
    {
      if(((can_rxbuf_struct.id & ~FILTER_MASK_EXT_ID) == FILTER_CODE_EXT_ID) \
        &&(can_rxbuf_struct.id_type == CAN_ID_EXTENDED))
      {
        at32_led_toggle(LED2);
      }
      else if(((can_rxbuf_struct.id & ~FILTER_MASK_STD_ID) == FILTER_CODE_STD_ID) \
             &&(can_rxbuf_struct.id_type == CAN_ID_STANDARD))
      {
        at32_led_toggle(LED3);
      }
      else{
        at32_led_toggle(LED4);  /* error: other identifier will be filter, can not be received! */
      }
    }
  }
